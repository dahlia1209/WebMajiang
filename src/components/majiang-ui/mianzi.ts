...
export function mianzi(pai:any) {
    ...
    return function(m:any) {
        ...
        if (m.replace(/0/,'5').match(/^[mpsz](\d)\1\1\1$/)) {
            ...
            let pai_r   = $('<span class="rotate">')
                            .append(jiagang ? nn.slice(-2).map((n:any)=>pai(s+n))
                                            : nn.slice(-1).map((n:any)=>pai(s+n)));
            let pai_l   = (! jiagang && nn.length == 4)
                                            ? nn.slice(1, 3).map((n:any)=>pai(s+n))
                                            : nn.slice(1, 2).map((n:any)=>pai(s+n));
            let label   = (  d == '+' ? 'シモチャから'
                           : d == '=' ? 'トイメンから'
                           :            'カミチャから' )
                        + (  jiagang        ? 'カカン'
