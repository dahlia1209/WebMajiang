...
function make_stat(paipu_list:any) {
...
function player_stat(stat:any, paipu:any, id:any) {
    if (! stat) {
        stat = {
            n_game:     0,
            n_rank:     [ 0, 0, 0, 0 ],
    ...
    for (let log of paipu.log) {
        ...
        let data = log.find((data:any) => data.hule && data.hule.l == l);
        if (data) {
            stat.n_hule++;
            stat.sum_defen += + data.hule.defen;
        }
        if (log.find((data:any) => data.hule && data.hule.baojia == l)) {
            stat.n_baojia++;
        }
        if (log.find((data:any) => data.dapai && data.dapai.l == l
                && data.dapai.p.slice(-1) == '*'))
        {
            stat.n_lizhi++;
        }
        if (log.find((data:any) => data.fulou && data.fulou.l == l)) {
            stat.n_fulou++;
        }
    }
    stat.n_game++;
...
function make_table(player:any) {
    let table = [];
...
function format(n:any, r:any, f:any) {
    let s = n.toFixed(r);
...
export class PaipuStat {
    public _root:any;
    public _tr:any;
    public _table:any;
    public _order:any;

    constructor(root:any, paipu_list:any, callback:any) {
        ...
        let cut_off = (Math.max(... this._table.map((x:any) => x[1])) / 5) | 0;
    ...
    sort(i:any) {
        ...
        this._table = this._table.sort(
                        (x:any, y:any)=> this._order > 0 ? x[i] - y[i] : y[i] - x[i]);
        return this;
    }
    ...
    show() {
        ...
        tbody.empty();
        for (let stat of this._table.filter((r:any) => r[1] > cut_off)) {
