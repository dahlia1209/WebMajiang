...
const rule:any = Majiang.rule();

let pref:any;

function repair_shan(shan:any, shoupai:any) {
...
function qipai(paistr:any) {
    ...
    view.shoupai = new Majiang.UI.Shoupai(
                                $('.shoupai'), view.pai, model.shoupai,null
                            ).redraw(true);

    model.he = new Majiang.He();
    view.he  = new Majiang.UI.He($('.he'), view.pai, model.he,null).redraw(true);
...
function paili(start:any) {
    ...
    for (let p of model.shoupai.get_dapai()) {
        ...
        p = p[0] + (+p[1]||5);
        if (dapai.find((dapai:any) => dapai.p == p)) continue;

        let tingpai:any = Majiang.Util.tingpai(shoupai);
        let n = tingpai.map((p:any) => 4 - model.shoupai._bingpai[p[0]][p[1]])
                       .reduce((x:any, y:any)=> x + y, 0)

        dapai.push({ p: p, tingpai: tingpai, n: n });
    }

    const cmp = (a:any, b:any) => b.n - a.n
    ...
    for (let d of dapai.sort(cmp)) {
        let html = '<div>打: '
                 ...
                 + ' 摸: '
                 + d.tingpai.map(
                     (p:any) => $('<span>').append(view.pai(p)).html()
                 ).join('')
                 + ` (${d.n}枚)</div>`;
        $('.paili').append($(html));
    }
...
$(function(){
    ...
    const p:any=localStorage.getItem('Majiang.pref')
    pref = JSON.parse(p);
