...
function init_analyzer(paistr:any, zhuangfeng:any, menfeng:any, baopai:any, hongpai:any) {
...
function submit() {
    ...
    let paistr:any = $('input[name="paistr"]').val();
    if (! paistr) return false;

    const z:any=$('select[name="zhuangfeng"]')
    const m:any=$('select[name="menfeng"]')
    let zhuangfeng = + z.val();
    let menfeng    = + m.val();
    ...
    if (shoupai._zimo) {
        if (shoupai._zimo.length == 2)
                analyzer.action_zimo({ l: menfeng, p: shoupai._zimo },null);
        else    analyzer.action_fulou({ l: menfeng, m: shoupai._zimo });
    }
    new Majiang.UI.Shan($('.shan'), pai, analyzer.shan).redraw();
    new Majiang.UI.Shoupai($('.shoupai'), pai, analyzer.shoupai,null).redraw(true);
