...
function markup(paistr:any, w:any, h:any) {
...
function parse(text:any, w:any, h:any) {
    return text.replace(/\\.|{(.+?)}/g, (match:any, mark:any)=>
...
$(function(){
    ...
    $('form').on('submit', ()=>{
        const size:any=$('input[name="size"]:checked')
        let [ , w, h ] = size.match(/^(\d+)x(\d+)$/);
