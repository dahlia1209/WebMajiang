...
function submit() {
    ...
    let baopai:any   = $.makeArray($('input[name="baopai"]'))
                        .map(n => Majiang.Shoupai.valid_pai($(n).val() as any))
                        .filter(p => p);
    let fubaopai:any = $.makeArray($('input[name="fubaopai"]'))
                        .map(n => Majiang.Shoupai.valid_pai($(n).val() as any))
                        .filter(p => p);

    const lc:any=$('input[name="lizhi"]:checked')
    let lizhi = + lc.val() || 0;

    let rule:any = $('select[name="rule"]').val();
    rule = ! rule      ? {}
    ...
    const tc:any=$('input[name="tianhu"]:checked')
    let param:any = {
        rule:       rule,
        zhuangfeng: + z.val(),
        menfeng:    + m.val(),
