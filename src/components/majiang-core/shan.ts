...
export class Shan {
    ...
    public _rule: Rule;
    public _pai: string[];
    public _baopai: string[];
    public _fubaopai: string[] | null;
    public _weikaigang: boolean;
    public _closed: boolean;

    static zhenbaopai(p:string) {
    ...
    zimo() {
        if (this._closed)     throw new Error(JSON.stringify(this));
        if (this.paishu == 0) throw new Error(JSON.stringify(this));
        if (this._weikaigang) throw new Error(JSON.stringify(this));
    ...
    gangzimo() {
        ...
        if (this.paishu == 0)         throw new Error(JSON.stringify(this));
        if (this._weikaigang)         throw new Error(JSON.stringify(this));
        if (this._baopai.length == 5) throw new Error(JSON.stringify(this));
