...
function _xiangting(m:number, d:number, g:number, j:number) {
...
function mianzi(bingpai:number[], n = 1) {

    if (n > 9) return dazi(bingpai);

    let max:any = mianzi(bingpai, n+1);
...
function mianzi_all(shoupai:any, jiangpai:any) {
...
function xiangting_yiban(shoupai:any) {

    let min = mianzi_all(shoupai,undefined);
...
function xiangting_guoshi(shoupai:any) {
...
function xiangting_qidui(shoupai:any) {
...
function xiangting(shoupai:Majiang.Shoupai) {
    return Math.min(
...
function tingpai(shoupai:Majiang.Shoupai, f_xiangting = xiangting) {
