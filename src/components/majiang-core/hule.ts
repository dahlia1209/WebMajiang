...
import { getRules, type RuleConfig } from './rule'
...
function mianzi(s: any, bingpai: any, n = 1): any {
...
function mianzi_all(shoupai: any) {
    ...
    let fulou = shoupai._fulou.map((m: string) => m.replace(/0/g, '5'));

    return shupai_all.map(shupai => [...shupai, ...zipai, ...fulou]);
}

function add_hulepai(mianzi: any, p: any) {
...
function hule_mianzi_yiban(shoupai: any, hulepai: any) {
...
function hule_mianzi_qidui(shoupai: any, hulepai: any) {
...
function hule_mianzi_guoshi(shoupai: any, hulepai: any) {
...
function hule_mianzi_jiulian(shoupai: any, hulepai: any) {
...
function hule_mianzi(shoupai: any, rongpai: any) {
    ...
    let hule_mianzi_yiban_result = hule_mianzi_yiban(new_shoupai, hulepai);
    let hule_mianzi_qidui_result = hule_mianzi_qidui(new_shoupai, hulepai);
    let hule_mianzi_guoshi_result = hule_mianzi_guoshi(new_shoupai, hulepai);
    let hule_mianzi_jiulian_result = hule_mianzi_jiulian(new_shoupai, hulepai);
...
function get_hudi(mianzi: any, zhuangfeng: any, menfeng: any, rule: any) {
...
function get_pre_hupai(hupai: any) {
...
function get_hupai(mianzi: any, hudi: any, pre_hupai: any, post_hupai: any, rule: any) {
    ...
    function yibeikou() {
        ...
        let beikou = shunzi.m.concat(shunzi.p).concat(shunzi.s)
            .map((x: any) => x >> 1).reduce((a: any, b: any) => a + b);
    ...
    function hunyise() {
        for (let s of ['m', 'p', 's']) {
            ...
            if (mianzi.filter((m: any) => m.match(yise)).length == mianzi.length
                && hudi.n_zipai > 0)
    ...
    function dasanyuan() {
        ...
        if (kezi.z[5] + kezi.z[6] + kezi.z[7] == 3) {
            let bao_mianzi = mianzi.filter((m: any) =>
                m.match(/^z([567])\1\1(?:[\+\=\-]|\1)(?!\!)/));
    ...
    function lvyise() {
        if (mianzi.filter((m: any) => m.match(/^[mp]/)).length > 0) return [];
        if (mianzi.filter((m: any) => m.match(/^z[^6]/)).length > 0) return [];
        if (mianzi.filter((m: any) => m.match(/^s.*[1579]/)).length > 0) return [];
...
function get_post_hupai(shoupai: any, rongpai: any, baopai: any, fubaopai: any) {
...
function get_defen(fu: any, hupai: any, rongpai: any, param: any) {
    ...
    if (hupai[0].fanshu[0] == '*') {
        ...
        damanguan = !param.rule['役満の複合あり'] ? 1
            : hupai.map((h: any) => h.fanshu.length).reduce((x: any, y: any) => x + y);
        base = 8000 * damanguan;

        let h = hupai.find((h: any) => h.baojia);
        if (h) {
            const baojia = h.baojia as '+' | '=' | '-'
            baojia2 = (menfeng + { '+': 1, '=': 2, '-': 3 }[baojia]) % 4;
            base2 = 8000 * Math.min(h.fanshu.length, damanguan);
        }
    }
    else {
        fanshu = hupai.map((h: any) => h.fanshu).reduce((x: any, y: any) => x + y);
    ...
    if (baojia2 != null && typeof base2 === 'number') {
...
function hule(shoupai: any, rongpai: any, param: any) {
