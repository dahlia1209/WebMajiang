...
export class Shoupai {
    public _bingpai: BingPai;
    public _fulou: string[];
    public _zimo: string | null;
    public _lizhi: boolean;

    static valid_pai(p: string) {
        if (p.match(/^(?:[mps]\d|z[1-7])_?\*?[\+\=\-]?$/)) return p;
    }

    static  valid_mianzi(m: string) {
        ...
        if (h.match(/^[mpsz](\d)\1\1[\+\=\-]\1?$/)) {
            ...
            const mmatch=m.match(/\d(?![\+\=\-])/g)
            if (mmatch==null) return;
            return m[0] + mmatch.sort().reverse().join('') +
                (m.match(/\d[\+\=\-]$/) || [''])[0];
        } else if (h.match(/^[mps]\d+\-\d*$/)) {
            let hongpai = m.match(/0/);
            const nnmatch=h.match(/\d/g)
            if (nnmatch==null) return; 
            let nn = nnmatch.sort();
            if (nn.length != 3) return;
            if (+nn[0] + 1 != +nn[1] || +nn[1] + 1 != +nn[2]) return;
            const hmatch=h.match(/\d[\+\=\-]?/g) 
            if(hmatch) h =   h[0] + hmatch.sort().join('');
    ...
    constructor(qipai: string[] = []) {
        ...
        for (let p of qipai) {
            ...
            const validatedPai = Shoupai.valid_pai(p);
            if (!validatedPai) throw new Error(`Invalid tile: ${p}`);
            const s: 'm' | 'p' | 's' | 'z' = validatedPai[0] as 'm' | 'p' | 's' | 'z';
            const n: number = parseInt(validatedPai[1], 10);

            if (this._bingpai[s][n] === 4) throw new Error(`Too many tiles of ${p}`);
            this._bingpai[s][n]++;
    ...
    static fromString(paistr = '') {
        ...
        let bingpai: string = fulou.shift() || '';;
        ...
        let zimo = (qipai.length - 2) % 3 === 0 ? qipai.slice(-1)[0] : null;
        const shoupai = new Shoupai(qipai);

        let last;
        for (let m of fulou) {
            if (!m) { shoupai._zimo = last ?? null; break }
            const validMianzi = Shoupai.valid_mianzi(m);
            if (validMianzi) {
    ...
    decrease(s: 'm' | 'p' | 's' | 'z', n: number) {
        let bingpai = this._bingpai[s];
        if (bingpai[n] == 0 || n == 5 && bingpai[0] == bingpai[5]) {
            if (this._bingpai._ == 0) throw new Error(`${JSON.stringify(this)},${s + n}`);
            this._bingpai._--;
        }
        else {
            bingpai[n]--;
    ...
    zimo(p: string, check = true) {
        if (check && this._zimo) throw new Error(`${JSON.stringify(this)},${p}`);
        if (p == '_') {
            this._bingpai._++;
            this._zimo = p;
        }
            ...
            let s = p[0] as 'm' | 'p' | 's' | 'z', n = +p[1];
            let bingpai = this._bingpai[s];
            if (bingpai[n] == 4) throw new Error(`${JSON.stringify(this)},${p}`);
            bingpai[n]++;
            if (n == 0) {
                if (bingpai[5] == 4) throw new Error(`${JSON.stringify(this)},${p}`);
                bingpai[5]++;
            }
            this._zimo = s + n;
        }
    ...
    fulou(m: string, check = true) {
        if (check && this._zimo) throw new Error(`${JSON.stringify(this)},${m}`);
        if (m != Shoupai.valid_mianzi(m)) throw new Error(m);
        if (m.match(/\d{4}$/)) throw new Error(`${JSON.stringify(this)},${m}`);
        if (m.match(/\d{3}[\+\=\-]\d$/)) throw new Error(`${JSON.stringify(this)},${m}`);
        let s = m[0] as 'm' | 'p' | 's' | 'z';
    ...
    gang(m: string, check = true) {
        if (check && !this._zimo) throw new Error(`${JSON.stringify(this)},${m}`);
        if (check && this._zimo && this._zimo.length > 2) throw new Error(`${JSON.stringify(this)},${m}`);
        if (m != Shoupai.valid_mianzi(m)) throw new Error(m);
        let s = m[0] as 'm' | 'p' | 's' | 'z';
        if (m.match(/\d{4}$/)) {
            for (let n of m.match(/\d/g) ?? []) {
                this.decrease(s, parseInt(n, 10));
            }
            this._fulou.push(m);
        }
            ...
            if (i < 0) throw new Error(`${JSON.stringify(this)},${m}`);
            this._fulou[i] = m;
            this.decrease(s,  parseInt(m.slice(-1), 10));
        }
        else throw new Error(`${JSON.stringify(this)},${m}`);
        this._zimo = null;
        return this;
    }
    ...
    get_dapai(check = true) {
        ...
        let deny: DenyMap = {};
        if (check && this._zimo && this._zimo.length > 2) {
            let m = this._zimo;
            let s  = m[0] as 'm' | 'p' | 's' | 'z';
            let match = m.match(/\d(?=[\+\=\-])/);
            let n = match ? +match[0] : 5; 
            deny[s + n] = true;
    ...
    get_chi_mianzi(p: string, check = true) {

        if (this._zimo) return null;
        if (!Shoupai.valid_pai(p)) throw new Error(p);

        let mianzi: any = [];
        let s =p[0] as 'm' | 'p' | 's' | 'z', n = + p[1] || 5, d: string | null = p.match(/[\+\=\-]$/)?.[0] ?? null;
    ...
    get_gang_mianzi(p: string) {
        ...
        if (p) {
            ...
            let s = p[0] as 'm' | 'p' | 's' | 'z', n = + p[1] || 5, d = p.match(/[\+\=\-]$/);
            if (!d) throw new Error(p);
            if (this._lizhi) return mianzi;
