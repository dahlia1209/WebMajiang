...
export class SuanPai {
    public _paishu:any;
    public _zhuangfeng:any;
    public _menfeng:any;
    public _baopai:any;
    public _dapai:any;
    public _lizhi:any;

    constructor(hongpai:any) {
    ...
    make_paijia(shoupai:any) {

        let n_suit:any = {};
        for (let s of ['m','p','s','z']) {
            n_suit[s] = shoupai._bingpai[s].slice(1).reduce((x:any, y:any)=> x + y);
        }
        let n_sifeng  = shoupai._bingpai.z.slice(1,5).reduce((x:any, y:any)=> x + y);
        let n_sanyuan = shoupai._bingpai.z.slice(5).reduce((x:any, y:any)=> x + y);
        ...
        return (p:any)=> paijia[p] ?? ( paijia[p] = this.paijia(p)
    ...
    suan_weixian(p:any, l:any, c:any) {
    ...
    suan_weixian_all(bingpai:any) {
        ...
        if (weixian_all) return (p:any)=>weixian_all[p[0]+(+p[1]||5)];
    }
}
